...
def make_freq_spectrum_with_speeds(config, model, speeds):
    ...
    filepath = os.path.join("Unfiltered_spectrums", "total_spectrum_y_values_"+run+".png")
    plt.savefig(filepath, dpi=300)
    plt.close()
    ...
    filepath = os.path.join("Filtered_spectrums", "total_spectrum_filtered_values_"+run+".png")
    plt.savefig(filepath, dpi=300)
    plt.close()

    for i in range(0, len(speeds)):
        plt.figure(figsize=(9, 6)) 
        plt.plot(freqs[i], X_mags[i], 'k-', label="Unfiltered", color='blue')
        plt.plot(freq_filtereds[i], X_filtered_mags[i], 'k-', label="Filtered", color='red')
        plt.xlim(0, 10)
        plt.xlabel('Frequency (Hz)', fontsize=18)
        plt.ylabel('Magnitude', fontsize=18)
        plt.legend()
        plt.title(f'Frequency spectrum comparison, {config}_{model}, speed {speeds[i]}', fontsize=18)
        plt.grid()

        os.makedirs("Spectrum_comparison", exist_ok=True) 
        filepath = os.path.join("Spectrum_comparison", "spectrum_comparison_"+run+".png")
        plt.savefig(filepath, dpi=300)
        plt.close()
